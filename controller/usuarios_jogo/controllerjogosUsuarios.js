...
const UsuarioDAO = require('../../model/DAO/usuarios.js')//alterar

//função para inserir um novo usuario
const inserirUsuarios = async function(Usuario, contentType){
    try {

        if(contentType == 'application/json'){ //alterar jogo por usuario
        if
        (Usuario.nome  == undefined || Usuario.nome  == '' || Usuario.nome   == null  || Usuario.nome.length > 80  ||
         Usuario.email == undefined || Usuario.email == '' || Usuario.email  == null  || Usuario.nome.length > 80  ||
         Usuario.senha == undefined || Usuario.senha == '' || Usuario.senha  == null  || Usuario.nome.length > 80  
            ...
            let resultUsuario = await UsuarioDAO.inserirUsuarios(Usuario)
